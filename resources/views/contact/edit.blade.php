...
<div class="col-md-3">
    <div class="form-group">
        <label for="municipality_select">Municipio:</label>
        <select id="municipality_select" class="form-control">
            <option value="">Seleccione municipio</option>
        </select>
        {!! Form::hidden('custom_field1', $contact->custom_field1, ['id' => 'custom_field1']); !!}
        {!! Form::hidden('custom_field2', $contact->custom_field2, ['id' => 'custom_field2']); !!}
    </div>
</div>
    ...
    $(document).ready(function() {
        ...
        function loadMunicipalities() {
            if ($('#department_select').length === 0) return;
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    allMunicipalities = response.data;

                    let departments = {};
                    allMunicipalities.forEach(function(m) {
                        if (m.department && !departments[m.department.id]) {
                            departments[m.department.id] = m.department.name;
                        }
                    });

                    let $deptSelect = $('#department_select');
                    $deptSelect.html('<option value="">Seleccione departamento</option>');
                    $.each(departments, function(id, name) {
                        $deptSelect.append('<option value="' + id + '">' + name + '</option>');
                    });

                    let savedMunicipalityId = $('#custom_field2').val();
                    if (savedMunicipalityId) {
                        let saved = allMunicipalities.find(m => m.id == savedMunicipalityId);
                        if (saved && saved.department) {
                            $deptSelect.val(saved.department.id).trigger('change');
                            setTimeout(function() {
                                $('#municipality_select').val(savedMunicipalityId);
                            }, 200);
                        }
                    }
                    ...
                    $deptSelect.off('change').on('change', function() {
                        let deptId = $(this).val();
                        let $munSelect = $('#municipality_select');
                        $munSelect.html('<option value="">Seleccione municipio</option>');
                        if (!deptId) return;
                        let filtered = allMunicipalities.filter(m => m.department && m.department.id == deptId);
                        filtered.forEach(function(m) {
                            $munSelect.append('<option value="' + m.id + '">' + m.name + '</option>');
                        });
                    });

                    $('#municipality_select').off('change').on('change', function() {
                        let munId = $(this).val();
                        let municipality = allMunicipalities.find(m => m.id == munId);
                        if (municipality) {
                            $('#custom_field1').val(municipality.code);
                            $('#custom_field2').val(municipality.id);
                        }
                    });
                },
                error: function(xhr) {
